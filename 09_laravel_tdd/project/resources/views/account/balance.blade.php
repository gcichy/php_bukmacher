...
<div class="relative justify-center" style="justify-content: center; width:50%; margin: 200px auto">
    ...
    <h1>Doładuj konto</h1>
    <img src="{{asset('img/blik.png')}}">
    <form method = "post" action="{{route('account.balance')}}">
        @csrf
        <div>
            <x-input-label  class='in' id="amount" for="amount" :value="__('Kwota')" />
            <x-text-input id="amount" class="block mt-1 w-full" type="text" name="amount" min="0" max="1000" autofocus />
            <x-input-error :messages="$errors->get('amount')" class="mt-2" />
        </div>
        <div>
            <x-input-label class='in' id="blikCode" for="blikCode" :value="__('Kod Blik')" />
            <x-text-input id="blikCode" class="block mt-1 w-full" type="text" name="blikCode" autofocus />
