...
<div class="relative justify-center" style="justify-content: center; width:50%; margin: 200px auto">
    ...
    <form method="POST" action="{{ route('account.premium.buy') }}">
        @csrf
        <!-- Statute -->
        <div>
            <!-- <x-input-label for="terms" :value="__('Zaakceptuj')" />-->
            <x-text-input id="statute" type="checkbox" name="statute"/>
            <a href="{{ url('/#rules') }}" class="statute-link">Zaakceptuj Regulamin</a>
            <x-input-error :messages="$errors->get('statute')" class="mt-2" />
        </div>
        ...
        <!-- User_deposit -->
        <div>
            <input id="deposit" name="deposit" type="hidden" value="{{$user->deposit}}">
