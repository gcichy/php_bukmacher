<?php $s = str_repeat('0', count($matches));  ?>
    <!DOCTYPE html>
...
<div class="relative justify-center" style="justify-content: center; width:50%; margin: 200px auto auto;">
    ...
    @foreach($matches as $match)
        ...
        <div id="match{{$i}}">
            ...
            <div id="buttons{{$i}}" class="bet-button-div">
                <a id="A1_{{$match["id"]}}" class="abutton" onclick="addToCoupon('A1_{{$match["id"]}}')">{{$match["answer_1"]}} <span>{{$odd["win_op_1"]}}</span></a>
                <a id="A2_{{$match["id"]}}" class="abutton" onclick="addToCoupon('A2_{{$match["id"]}}')">{{$match["answer_2"]}} <span>{{$odd["win_op_2"]}}</span></a>
            </div>
        </div>
        <br>
        <?php $i+=1 ?>
    ...
    @else
    <div style="text-align: center;">
        <form method="POST" action="{{route("specialoffers.bet")}}">
            @csrf
            <div>
